...
function manageShortage(managedItem, initial_item_slot, final_item_slot)
	...
	textOutput("Type anything if all the "..(itemNames[managedItem+3]).." slots are refilled (slots "..(initial_item_slot).." to "..(final_item_slot)..").", 1, 7, 0)
	begin = securedInput(37, 7, "text", 0, 1, " ", " ")
...
function inventoryManagement(refuel_count,Right_or_Left,throw_cobble) 
	...
	if Chest_approval then
		...
		if (turtle.getItemCount(16) ~= 0) and (m~=16) then
			if fullInventory(m)==17 then --[[If inventory is full, throw garbage]]--
				if throw_stuff then
				...
				if not(throw_stuff) or ((thrown_slots <= 2) and (fullInventory(n)>15)) then --[[Here]]--
					if shortage[3] then
...
if (io.open("favorite", "r") == nil) or ((io.open("favorite", "r") ~= nil) and (Favorite_approval == "n")) then
	...
	textOutput("Use regular chests? Yes=1 No=0", 1, 4, 0)
	chest_use_input = securedInput(37, 4, "integer", 0, 1, " ", " ")	
	if (chest_use_input > 0) then
		Chest_approval = true
	else	
		Chest_approval = false
	end

	textOutput("Use enderchests?", 1, 5, 0)
	enderchest_input = securedInput(37, 5, "integer", 0, 1, " ", " ")	
	if (enderchest_input > 0) then
		enderchest = true
	else	
		enderchest = false
	end

	textOutput("Use torches?", 1, 6, 0)
	torch_input = securedInput(37, 6, "integer", 0, 1, " ", " ")
	if (torch_input > 0) then
		Torch_approval = true
	else	
		Torch_approval = false
	end	

	textOutput("Throw away specific items?", 1, 7, 0)
	thow_away_input = securedInput(37, 7, "integer", 0, 1, " ", " ")
	if (thow_away_input > 0) then
		throw_stuff = true
	else	
		throw_stuff = false
	end	
	...
	New_favorite = securedInput(1, 6, "text", 0, 0, "y", "n")
...
textOutput("Start?. Type any number,", 1, 11, 0)
start_miner = securedInput(37, 11, "integer", -100000, 100000, " ", " ")
resetScreen()
